...
export async function getHomepageContent(): Promise<HomepageContent> {
  const [trending, popularMovies, popularTV, upcoming, topRated] = await Promise.all([
...
async function fetchTMDBData(endpoint: string) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Failed to fetch ${endpoint}. Please try again.`);
...
function renderSection(title: string, items: MediaItem[], type: string): string {
  return `
    <section class="mb-12 animate-fade-in">
      <h2 class="text-2xl text-white text-center font-bold mb-8">${title}</h2>
      <div id="${type}-items-container" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        ${items.slice(0, 5).map(item => renderMediaCard(item)).join("")}
      </div>
  ...
  return `
    <div class="card bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:scale-105 transition-transform duration-300 animate-fade-in">
      ...
      <div class="p-4 text-white">
        ...
        <p class="text-sm mb-2">Genres: ${item.genre_ids.map((id: number) => getGenreName(id)).join(", ")}</p>
...
function setupEventListeners() {
  ...
  sections.forEach(section => {
    ...
    showMoreButton?.addEventListener("click", async () => {
      ...
      let moreItems;
      if (section === "trending") {
        moreItems = await fetchTMDBData(`/trending/all/day?page=${currentPage}`);
      ...
      if (itemsContainer) {
        itemsContainer.innerHTML += moreItems.results.map((item: MediaItem) => renderMediaCard(item)).join("");
      }
    });
  });
}